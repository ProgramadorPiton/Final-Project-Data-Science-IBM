...
def get_pie_chart(entered_site):
    ...
    success_fail_count = filtered_df['class'].value_counts()
    
    fig = px.pie(
        names=success_fail_count.index, 
        values=success_fail_count.values, 
        title=title,
...
def get_scatter_plot(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        title = 'Scatter Plot Payload for All Sites'
    else:
        filtered_df = spacex_df[(spacex_df['Launch Site'] == entered_site) & 
                                 (spacex_df['Payload Mass (kg)'] >= min_payload) & 
                                 (spacex_df['Payload Mass (kg)'] <= max_payload)]
        title = f'Scatter Plot Payload for {entered_site}'
